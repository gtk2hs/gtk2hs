...
main = do
  args <- getArgs
  if (length args/=3) then usage else do
    let [typesFile, bootPath, outFile] = args
    generateHooks typesFile bootPath outFile

generateHooks :: String -> String -> String -> IO ()
generateHooks typesFile bootPath outFile = do
...
usage = do
 putStr "Program to generate callback hook for Gtk signals. Usage:\n\
	\HookGenerator <signatureFile> <bootPath> <outFile>\n\
	\where\n\
...
generate sig = let ident = mkIdentifier sig in
  indent 0.ss "type Tag_".ident.ss " =".
  indent 1.ss "Ptr () -> ".mkRawtype sig.
  indent 0.
  indent 0.ss "foreign".ss " export dynamic mkHandler_".ident.ss " ::".
  ...
  indent 1.ss "ConnectAfter -> obj -> IO (ConnectId obj)".
  indent 0.ss "connect_".ident.ss " signal".
  mkArg sig.
  indent 1.ss "user after obj =".
  indent 1.ss "do".
